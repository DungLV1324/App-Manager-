...
import com.example.filemanager.ui.custom.model.Data;
import com.example.filemanager.ui.custom.model.MyData;
import com.example.filemanager.util.DimensionUtil;
...
public class HorizontalBarChartView extends View {
    ...
    private float corner_radius; // radius
    ...
    public void init(@Nullable AttributeSet set) {
        ...
        corner_radius = DimensionUtil.dpToPx(12);
        tb_left_radius = new float[]{corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius}; // Is only round on top-bottom left.
        flat_radius = new float[]{corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius}; // A flat square. Without rounded corners.
        tb_right_radius = new float[]{corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius}; // Is only round on top-bottom right.
...
//        Timber.e("tunglt: fullWidth: " + fullWidth + "---- width screen: " + ViewUtil.getWidthScreen());
        ...
        for (int i = dataList.size() - 1; i >= 0; i--) {
            ...
            float newWidth = (fullWidth * data.getPercentage() / 100F) + (i == 0 ? 0 : corner_radius * 2.3f);
            bounds.top = 0;
            bounds.left = 0;
            bounds.right = (int) newWidth;
            bounds.bottom = fullHeight;
            currentWidth += newWidth - (i == 0 ? 0 : corner_radius * 2.3f);
            if (starting) {
            ...
            if ((int) currentWidth <= fullWidth) {
                ...
                canvas.translate(i == 0 ? 0 : newWidth - corner_radius * 2.3f, 0);

            } else {
                Log.e("HorizontalBarCV_ERR", "This actions, exceeds a total of 100%.\nDid not add new data.");
                return;
            }
//            Timber.e("tunglt: fullWidth: " + fullWidth + "---- currentWidth: " + currentWidth);
