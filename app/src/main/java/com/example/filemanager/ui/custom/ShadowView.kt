...
open class ShadowView @JvmOverloads constructor(
    ...
    init {
        val a = getContext().obtainStyledAttributes(
            attributeSet, R.styleable.ShadowView, defStyleInt, 0
        )
        ...
        shadowRadius =
            a.getDimensionPixelSize(R.styleable.ShadowView_shadowRadius, SIZE_DEFAULT).toFloat()
        ...
        if (shadowMargin >= 0) {
            ...
            shadowMarginTop =
                a.getDimensionPixelSize(R.styleable.ShadowView_shadowMarginTop, SIZE_DEFAULT)
            shadowMarginLeft =
                a.getDimensionPixelSize(R.styleable.ShadowView_shadowMarginLeft, SIZE_DEFAULT)
            shadowMarginRight =
                a.getDimensionPixelSize(R.styleable.ShadowView_shadowMarginRight, SIZE_DEFAULT)
            shadowMarginBottom =
                a.getDimensionPixelSize(R.styleable.ShadowView_shadowMarginBottom, SIZE_DEFAULT)
        }

        val cornerRadius =
            a.getDimensionPixelSize(R.styleable.ShadowView_cornerRadius, SIZE_UNSET).toFloat()
        if (cornerRadius >= 0) {
            ...
            cornerRadiusTL =
                a.getDimensionPixelSize(R.styleable.ShadowView_cornerRadiusTL, SIZE_DEFAULT)
                    .toFloat()
            cornerRadiusTR =
                a.getDimensionPixelSize(R.styleable.ShadowView_cornerRadiusTR, SIZE_DEFAULT)
                    .toFloat()
            cornerRadiusBL =
                a.getDimensionPixelSize(R.styleable.ShadowView_cornerRadiusBL, SIZE_DEFAULT)
                    .toFloat()
            cornerRadiusBR =
                a.getDimensionPixelSize(R.styleable.ShadowView_cornerRadiusBR, SIZE_DEFAULT)
                    .toFloat()
        }
        a.recycle()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (child.visibility != View.GONE) {
            ...
            maxWidth = if (shadowMeasureWidthMatchParent) Math.max(
            ...
            maxHeight = if (shadowMeasureHeightMatchParent) Math.max(
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        if (changed) foregroundDrawBoundsChanged = changed
    }

    private fun layoutChildren(
        left: Int, top: Int, right: Int, bottom: Int, forceLeftGravity: Boolean
    ) {
            ...
            if (child.visibility != View.GONE) {
                ...
                when (absoluteGravity and Gravity.HORIZONTAL_GRAVITY_MASK) {
                    Gravity.CENTER_HORIZONTAL -> childLeft =
                        parentLeft + (parentRight - parentLeft - width) / 2 + lp.leftMargin - lp.rightMargin + shadowMarginLeft - shadowMarginRight
                    Gravity.RIGHT -> {
                        if (!forceLeftGravity) {
                ...
                when (verticalGravity) {
                    ...
                    Gravity.CENTER_VERTICAL -> childTop =
                        parentTop + (parentBottom - parentTop - height) / 2 + lp.topMargin - lp.bottomMargin + shadowMarginTop - shadowMarginBottom
    ...
    private fun getShadowMarginMax() = intArrayOf(
        shadowMarginLeft, shadowMarginTop, shadowMarginRight, shadowMarginBottom
        ...
        foregroundDraw?.let {
            if (foregroundDrawBoundsChanged) {
                ...
                if (foregroundDrawInPadding) {
                    ...
                    selfBounds.set(
                        paddingLeft, paddingTop, w - paddingRight, h - paddingBottom
                    )
                }
                Gravity.apply(
    ...
    override fun drawableHotspotChanged(x: Float, y: Float) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) foregroundDraw?.let {
            it.setHotspot(
                x,
                y
            )
